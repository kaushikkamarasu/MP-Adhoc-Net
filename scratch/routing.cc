...
#include <sstream>   // for parsing comma-separated sink list
                           ...
                           uint32_t packetSize)
{
    ...
    // Build sender list excluding sink nodes
    ...
    // Shuffle senderIds using a deterministic engine seeded from ns-3 RNG manager
    unsigned seed_val = static_cast<unsigned>(RngSeedManager::GetSeed() + RngSeedManager::GetRun());
    std::default_random_engine rng_engine(seed_val);
    std::shuffle(senderIds.begin(), senderIds.end(), rng_engine);

    uint32_t totalSenders = senderIds.size();
    uint32_t heavyCount = std::max<uint32_t>(1, (uint32_t)std::floor(heavyFraction * totalSenders));
    uint32_t lightCount = (totalSenders > heavyCount) ? (totalSenders - heavyCount) : 0;

    // Compute rates so the heavy group contributes heavyTrafficShare fraction of total expected traffic.
    ...
    // Random variables for intervals
    ...
    // Uniform RVs for jitter and sink selection
    Ptr<UniformRandomVariable> uniform = CreateObject<UniformRandomVariable>();
    Ptr<UniformRandomVariable> sinkRand = CreateObject<UniformRandomVariable>();
    ...
    for (uint32_t idx = 0; idx < totalSenders; ++idx)
    {
        ...
        // choose a random destination that is not the sender
        uint32_t destNode;
        do {
            destNode = randDest->GetInteger(0, nodes.GetN() - 1);
        } while (destNode == nodeId);


        UdpClientHelper client(ifaces.GetAddress(destNode), port);
...
static std::vector<double> g_initialEnergyJ;
 ...
 int main(int argc, char *argv[])
 {
   ...
   uint32_t numNodes = 5;
   double simTime = 7200.0; // Simulation time in seconds
  ...
  double meanLightIntervalSeconds = 10.0;   // mean interval for light senders
  uint32_t maxPacketsPerSender = 1000;      // cap per-sender packets
  ...
  RngSeedManager::SetRun(rngRun);
   ...
   wifiChannel.AddPropagationLoss("ns3::LogDistancePropagationLossModel",
    "Exponent", DoubleValue(3.0));  // Large-scale fading

wifiChannel.AddPropagationLoss("ns3::NakagamiPropagationLossModel",
    "m0", DoubleValue(1.5),        // NLOS / obstructed
    "m1", DoubleValue(1.5),
    "m2", DoubleValue(1.0));       // optional
   ...
   wifiPhy.Set("RxSensitivity", DoubleValue(-95.0)); // dBm
  ...
  for (uint32_t i = 0; i < energySources.GetN(); ++i)
  {
      ...
      battery->SetAttribute("NominalVoltage", DoubleValue(3.7));      // Volts
      battery->SetAttribute("FullVoltage", DoubleValue(4.2));         // Volts typical Li-ion
      battery->SetAttribute("CutoffVoltage", DoubleValue(3.0));       // Volts
      battery->SetAttribute("NominalCapacity", DoubleValue(2.0));     // Amp-hours (2 Ah ~ realistic handheld)
      battery->SetAttribute("MaxCapacity", DoubleValue(2.0));         // Amp-hours
      battery->SetAttribute("InternalResistance", DoubleValue(0.05)); // Ohms (tune if needed)      
  }
  ...
  for (uint32_t i = 0; i < energySources.GetN(); ++i)
{
  ...
  for (uint32_t i = 0; i < nodes.GetN(); ++i)
  {
  ApplicationContainer s = sinkHelper.Install(nodes.Get(i));
  s.Start(Seconds(1.0));
...
// Gauss-Markov mobility parameters (smooth pedestrian-like motion)
Ptr<UniformRandomVariable> meanVel = CreateObject<UniformRandomVariable>();
meanVel->SetAttribute("Min", DoubleValue(1.0));
meanVel->SetAttribute("Max", DoubleValue(2.0));

// The GaussMarkov model expects PointerValue for the RVs and Box bounds
mobility.SetMobilityModel(
    "ns3::GaussMarkovMobilityModel",
    "Bounds", BoxValue(Box(0.0, 200.0, 0.0, 200.0, 0.0, 0.0)),
    "TimeStep", TimeValue(Seconds(1.0)),
    "Alpha", DoubleValue(0.85),
    "MeanVelocity", PointerValue(meanVel)
);
...
// --- NetAnim XML Output ---
std::string filename = "adhoc-animation-" + protocolName + ".xml";
AnimationInterface anim(filename);
anim.EnablePacketMetadata(false);


// Optional: Set node display properties
for (uint32_t i = 0; i < nodes.GetN(); ++i)
{
...
// Enable energy tracking (optional but useful)
for (uint32_t i = 0; i < energySources.GetN(); ++i)
{
    Ptr<energy::GenericBatteryModel> gb = DynamicCast<energy::GenericBatteryModel>(energySources.Get(i));
    if (gb)
    {
        anim.UpdateNodeSize(nodes.Get(i)->GetId(), 15, 15);
    }
}
